  ...
  if (isInitializing) {
    return (
      <div className="min-h-dvh bg-slate-50 dark:bg-slate-950 flex items-center justify-center" role="status">
        <Loader2 className="w-8 h-8 animate-spin text-red-800 dark:text-red-300" />
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl p-6 sm:p-8 space-y-6">
        ...
        {sessionMessage && (
          <div
            role="alert"
            className="flex items-center gap-2 bg-amber-50 dark:bg-amber-950/50 border border-amber-100 dark:border-amber-900 text-amber-800 dark:text-amber-300 text-xs p-3.5 rounded-xl"
          >
        ...
        {location.state?.passwordChanged && (
          <div
            role="status"
            className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-100 dark:border-emerald-900 text-emerald-700 dark:text-emerald-300 text-xs p-3.5 rounded-xl"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="login-correo" className="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1">
              Correo Electrónico *
            </label>
            <div className="relative">
              <Mail className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-400 pointer-events-none" />
              <input
                id="login-correo"
                type="email"
                required
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-800/20 dark:focus:ring-red-400/20 focus:border-red-800 text-slate-800 dark:text-slate-100 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-400 min-h-[44px]"
              />
            </div>
          </div>

          <div>
            <label htmlFor="login-contrasena" className="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1">
              Contraseña *
            </label>
            <div className="relative">
