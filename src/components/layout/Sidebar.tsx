...
export function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  const linkClasses = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-3.5 py-3 text-sm font-medium rounded-xl transition-colors min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 dark:focus-visible:ring-red-400 ${
      isActive
        ? "bg-red-100/40 dark:bg-red-950/40 text-red-800 dark:text-red-300 font-semibold"
        : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-950"
    }`;
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static top-0 bottom-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col justify-between h-dvh flex-shrink-0 transition-transform duration-300 ease-in-out ${
        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="p-4 sm:p-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-slate-800 dark:text-slate-100 text-sm leading-tight">Mantenimientos</h2>
            ...
            {onClose && (
              <button
                ...
                className="lg:hidden min-w-[44px] min-h-[44px] flex items-center justify-center text-slate-400 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 dark:focus-visible:ring-red-400"
              >
                <X className="w-5 h-5" />
              </button>
            )}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
          <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-950">
            <div className="flex items-center gap-2.5 overflow-hidden min-w-0">
              {/* Badge de Iniciales */}
              <div className="w-8 h-8 rounded-lg bg-red-100/40 dark:bg-red-950/40 text-red-700 dark:text-red-300 font-bold text-xs flex items-center justify-center flex-shrink-0">
                {iniciales}
              </div>

              {/* Nombre y Correo */}
              <div className="truncate min-w-0">
                <p className="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate" title={nombreMostrar}>
                  {nombreMostrar}
                </p>
                <p className="text-[10px] text-slate-400 dark:text-slate-400 truncate" title={user?.correo}>
            ...
            <button 
              ...
              className="min-w-[44px] min-h-[44px] flex items-center justify-center text-slate-400 dark:text-slate-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-rose-50 dark:hover:bg-rose-950/50 rounded-lg transition-colors flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 dark:focus-visible:ring-red-400"
            >
